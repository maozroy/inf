...
typedef int (*match_ptr)(const sl_node_t *node, void *param);
typedef int (*action_ptr)(sl_node_t *node, void *param);
...
/*The function inserts a new node after pos. you can only insert a node which
 it's next is null (meaning - a node which is not connected to other nodes)
 *The function returns a pointer to new_node.
 */
 ...
 * instead.
 * The functions changes the node passed, so you cannot use it afterwards.
 */
 ...
 * behavior.
 * The function assigns NULL to the next of the removed node.
 */  
 ...
 * Notice - if the nodes are in loop, the function will result in undefined
 * behavior */
int SListForEach(sl_node_t *node, void *param, action_ptr run_action);
