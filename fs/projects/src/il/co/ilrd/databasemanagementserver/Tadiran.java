...
//		prepareAndSendMessage(DatabaseKeys.DELETE_TABLE, "TestCountry");
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (true) {
					...
					try {
						receivedMssage = (ServerMessage) ConversionUtils.toObject(receiveBuffer.array());
					...
					System.out.println("response db name:" + dBreceivedMessage.getKey().getDatabaseName());
					System.out.println("response action type:" + dBreceivedMessage.getKey().getActionType());
	...
	private void sendMessage(DatabaseManagementMessage dBMessage) throws IOException {
		...
		byte[] serverMessageArray = ConversionUtils.toByteArray(serverMessage);
		buffer.clear();
